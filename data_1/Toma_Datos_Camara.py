...
while True:
    ...
    frame_copy = frame.copy()  # Crear una copia del frame para la ventana "Image"

    # Si hay un cuadro delimitador dibujado, mostrarlo en la ventana "Camera" y "Image"
    if bbox is not None:
        drawBox(frame, bbox)
    ...
    # Mostrar el frame de la imagen con el cuadro delimitador
    cv2.imshow("Image", frame_copy)

    # Si se presiona la tecla 'c', capturar la imagen y guardarla
    key = cv2.waitKey(1) & 0xFF
    if key == ord('c') and roi.size > 0:
        # Guardar la imagen como archivo PNG
        img_filename = f"data_1/image_{data_counter}.png"
        cv2.imwrite(img_filename, roi_resized)

        # Guardar el vector en un archivo TXT
        txt_filename = "data_1/vectors_1.txt"
        with open(txt_filename, 'a') as f:
            np.savetxt(f, [roi_vector], fmt='%d', delimiter=',')

        print(f"Imagen y datos guardados: {img_filename}, {txt_filename}")
        data_counter += 1
